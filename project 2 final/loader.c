...
void load_program(char *program_filename) {                   
    ...
    if (fgets(buffer, 32 + 2, prog) != NULL) {
        ...
        printf("Error occurred while loading data size from %s\n", program_filename);
        exit(EXIT_FAILURE);
    }
    ...
    // Load text segment
    i = 0;
    while (i < text_size && fgets(buffer, 32 + 2, prog) != NULL) {
        load_inst_to_mem(buffer, i);
        i += 4;  // Each instruction is 4 bytes
    }
    
    // Load data segment
    i = 0;
    while (i < data_size && fgets(buffer, 32 + 2, prog) != NULL) {
        load_data_to_mem(buffer, i);
        i += 4;  // Each data word is 4 bytes
    }
